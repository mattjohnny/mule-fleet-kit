...
function looksLikeClassName(value) {
    if (!value || value.length > MAX_ERROR_NAME)
        return false;
...
export function safeErrorName(error) {
    ...
    if (SAFE_ERROR_NAMES.has(candidate))
        return candidate;
 ...
 */
function stackBody(error, stack) {
    let message = "";
    try {
...
function isOursListener(listener) {
    return (typeof listener === "function" &&
        listener[FATAL_LISTENER_MARKER] === true);
}
/**
...
export function installProcessErrorHandlers() {
    ...
    process.on("unhandledRejection", markAsOurs((reason) => {
        handleFatal("unhandledRejection", "unhandled_rejection", reason);
    }));
    process.on("uncaughtException", markAsOurs((error) => {
 ...
 */
function foreignOwnerExists(signal) {
    ...
    const registered = process.listeners(signal);
 ...
 */
function handleFatal(signal, event, error) {
